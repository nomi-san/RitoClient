...
public static class Rundll32
{
        ...
        nint lpStartupInfo, nint lpProcessInformation)
    {
        ...
        int ret;
        using (var call = CreateProcessW.GetCall())
        {
            ret = call.Func(lpApplicationName, lpCommandLine,
        ...
        nint lpStartupInfo, nint lpProcessInformation)
    {
        ...
        int ret;
        using (var call = CreateProcessAsUserW.GetCall())
        {
            ret = call.Func(hToken, lpApplicationName, lpCommandLine,
                lpProcessAttributes, lpThreadAttributes, bInheritHandles,
                dwCreationFlags, lpEnvironment, lpCurrentDirectory, lpStartupInfo, lpProcessInformation);
        }
    ...
    [UnmanagedCallersOnly(EntryPoint = nameof(BootstrapEntryW))]
    public static int BootstrapEntryW(nint hwnd, nint inst, nint cmdLine, int _)
    {
    ...
    {
        var dllPath = Module.ThisModulePath;
...
#if DEBUG
        var mod = Native.GetModuleHandle(dllPath);
        var pa = Native.GetProcAddress(mod, nameof(Dummy));
        loader = Native.CreateRemoteThread(process, 0, 0, pa, IntPtr.Zero, 0, 0);
        Native.WaitForSingleObject(loader, uint.MaxValue);
        Native.CloseHandle(loader);
#endif
    }
    ...
    [UnmanagedCallersOnly(EntryPoint = nameof(Dummy))]
    public static void Dummy(nint _) { }
}